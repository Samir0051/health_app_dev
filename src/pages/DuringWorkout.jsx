...
export default function DuringWorkout() {
  const { currentSession, baselineTests, athlete, updateSession } =
    useAppData();
  const navigate = useNavigate();

  const groups = useMemo(
    () => groupIntoPhases(currentSession?.plan?.reps || []),
    [currentSession],
  );
    ...
    return (
      <div>
        <SectionHeader
          ...
          description="No active plan yet."
        />
        <div className="card p-6 text-center">
          <p className="text-sm text-mist mb-4">
            Build a workout plan first, then come back here to run it.
          </p>
  ...
  function logRest(actualRestSec) {
    ...
    updateSession(currentSession.id, {
      duringWorkout: { repLogs: updatedLogs },
    });
      ...
      {allDone ? (
        <div className="card p-6 text-center">
          ...
          <p className="text-paper font-medium mb-1">
            All planned reps logged.
          </p>
          <p className="text-sm text-mist mb-5">
            Head to post-workout to wrap up the session.
          </p>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="card p-5">
            <div className="flex items-center justify-between mb-4">
              <CardHeading
                title={`Rep ${groupIndex + 1} of ${groups.length}`}
                dotColor="track"
              />
              <span className="text-xs text-mist">
                {group.length > 1
                  ? `${group.length}-phase merged run`
                  : "single rep"}
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mb-5">
              {group.map((rep, ri) => (
                <span
                  key={rep.id}
                  className="num rounded-md bg-ink-soft border border-surface-hairline px-2.5 py-1 text-xs text-mist"
                >
            ...
            {step === STEPS.REACTION && (
              <div className="space-y-3">
                <CardHeading
                  title="Inter-rep reaction test"
                  dotColor="signal"
                />
                <p className="text-xs text-mist">
                  Same 5-tap protocol, scored against your personal baseline.
                </p>
                <ReactionTimeTest
                  demoCount={0}
                  repCount={5}
                  onComplete={onReactionComplete}
                  compareMeanMs={baselineMean}
                />
              </div>
            )}
            ...
            {step === STEPS.RECOMMEND && restRec && (
              <div className="space-y-3">
                ...
                <div
                  className={`rounded-xl border px-4 py-3 ${TONE_BG[restRec.deltaPct >= 10 ? "signal" : "gate"]}`}
                >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-lg bg-ink-soft px-3 py-2.5 text-center">
                    ...
                    <p className="num text-lg text-paper">
                      {formatSec(restRec.intendedRest)}
                    </p>
                  </div>
                  <div className="rounded-lg bg-ink-soft px-3 py-2.5 text-center">
                    <p className="text-[11px] uppercase tracking-wide text-mist">
                    ...
                    <p
                      className={`num text-lg ${TONE_TEXT[restRec.deltaPct >= 10 ? "signal" : "gate"]}`}
                    >
                      {formatSec(restRec.recommendedSec)}
                    </p>
                  </div>
                </div>
                <p className="text-xs text-mist/70">
                  You and your coach make the final call — this never overrides
                  your plan automatically.
                </p>
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => chooseRest(restRec.intendedRest)}
                  ...
                  <button
                    type="button"
                    onClick={() => chooseRest(restRec.recommendedSec)}
            ...
            <div className="card p-5">
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {repLogs.map((log, i) => (
                  <div
                    key={i}
                    className="rounded-lg bg-ink-soft px-3 py-2 text-xs"
                  >
